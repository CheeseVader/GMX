...
$Service = Join-Path $Backend "src\ai-vision-lab\localVisionService.js"
$Route   = Join-Path $Backend "src\routes\aiVisionLab.js"
$Server  = Join-Path $Backend "src\server.js"

foreach($p in @($Service,$Route,$Server)){
    if(!(Test-Path $p)){ throw "No existe: $p" }
}

$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$Backup = Join-Path $ProjectRoot "BACKUP-AI-VISION-LAB-LOCAL-R2.2-$stamp"
New-Item -ItemType Directory -Force -Path $Backup | Out-Null

Info "Creando respaldo R2.2"
foreach($f in @($Service,$Route,$Server)){
...
Info "Reescribiendo localVisionService.js con expresiones Unicode seguras"
...
WriteText $Service $localJs
Ok "localVisionService.js corregido sin caracteres mojibake en regex."

Info "Validando sintaxis Node"
...
Write-Host " GMX AI VISION LAB LOCAL R2.2 CORREGIDO" -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Green
Write-Host ""
Write-Host "La expresion regular con caracteres Ã... fue eliminada." -ForegroundColor Cyan
Write-Host "Ahora se usan propiedades Unicode seguras: \p{L}" -ForegroundColor Cyan
Write-Host ""
